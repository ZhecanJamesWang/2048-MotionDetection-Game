            ...
            if command==' ': #if the command becomes blank again then that means the face comes back into the square 
                ...
                if command!=' ':
                    ...
                    if command=="d":
                        ...
                        pygame.mixer.music.play()
                    # os.system('play --no-show-progress --null --channels 1 synth %s sine %f' % ( 0.5,250)) #0.5 is the duration, 250 is the frequency
                    # face_detection_2048_sub.game_2048_text_version(command) # outputing the direction commands to 2048 algorithm
